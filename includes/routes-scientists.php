...
function add_scientist($data)
{
    ...
    if (empty($name) || empty($position) || empty($work_place) || $academic_rank_id <= 0) {
...
function remove_scientist_image($data)
{
    ...
    $image = $wpdb->get_row(
        $wpdb->prepare("SELECT path FROM images WHERE scientist_id = %d", $id),
        ARRAY_A
    );
    ...
    // Delete from DB
    $wpdb->delete('images', ['scientist_id' => $id]);
